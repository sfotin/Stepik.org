...
# print(*(map(lambda x: x // 2 if x % 2 == 0 else x, filter(lambda x: not x % 2 and x <= 47, numbers))))

print(*map(lambda x: x // 2 if not x % 2 else x, filter(lambda x: x <= 47 or not x % 2, numbers)))
